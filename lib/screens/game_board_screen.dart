...
class _GameBoardScreenState extends State<GameBoardScreen>
  ...
  // --- NEW: action guard & board flip ---
  bool _isActionLocked = false;
  bool _isBoardFlipped = false;
  int? _hitFlashBar; // 1 = my bar flash, -1 = opp bar flash
  ...
  late Animation<double> _flipScaleX;
  ...
  void initState() {
    ...
    _diceRotation =
        Tween<double>(begin: -0.18, end: 0.18).animate(_diceController);

    // board flip scale animation (scaleX 1→0 then 0→1)
    _flipController = AnimationController(
      vsync: this,
      duration: const Duration(milliseconds: 220),
    );
    _flipScaleX = Tween<double>(begin: 1.0, end: 0.0).animate(
      CurvedAnimation(parent: _flipController, curve: Curves.easeIn),
    );
  ...
  void _initBoard() {
    ...
    // classic backgammon starting position
    _board[23] = 2;
    _board[12] = 5;
    _board[7] = 3;
    _board[5] = 5;
  ...
  // ─── board flip ───────────────────────────────────────────────────────────
  Future<void> _flipBoard() async {
    if (_flipController.isAnimating) return;
    // scale out
    await _flipController.forward();
    if (!mounted) return;
    // toggle state
    setState(() => _isBoardFlipped = !_isBoardFlipped);
    // scale back in
    if (!mounted) return;
  ...
  void _evaluateInitialRoll() {
    if (_myInitialRoll == _opponentInitialRoll) {
      ...
      setState(() {
        _centerMessage =
            iWon ? "ניצחת בהגרלה! אתה מתחיל." : "היריב מתחיל.";
  ...
  void _endTurn() {
    ...
    Future.delayed(const Duration(milliseconds: 800), () {
      if (!mounted) return;
      final nextTurn = (_currentTurnId == widget.myPlayFabId)
          ? widget.opponentId
          : widget.myPlayFabId;
      setState(() {
        _currentTurnId = nextTurn;
        _isActionLocked = false;
      });
      _startTurnTimer();
    ...
    Timer.periodic(const Duration(milliseconds: 85), (timer) {
      ...
      if (rolls >= 18) {
        ...
        setState(() {
          ...
          _availableMoves =
              (_die1 == _die2) ? [_die1, _die1, _die1, _die1] : [_die1, _die2];
        });
  ...
  void _executeBotMovesSequentially() {
    ...
    final move = BotService.selectBotMove(
        _board, List.from(_availableMoves), _oppBar, widget.botSkill!);
  ...
  void _executeBotMove(int source, int dest) {
    ...
    setState(() {
      if (dest == 24) {
        ...
        if (_availableMoves.contains(neededDie)) {
          ...
          final overshooting =
              _availableMoves.where((m) => m > neededDie).toList()..sort();
    ...
    if (hitBlot) {
      setState(() => _hitFlashBar = 1); // my bar flashes
  ...
  void _checkAndAutoMove() {
    if (!mounted ||
        _currentTurnId != widget.myPlayFabId ||
        _availableMoves.isEmpty ||
        _gameState != "playing" ||
        _isActionLocked) return;
  ...
  void _executeMove(int dest) {
    if (_isActionLocked && _selectedPoint == null) return;
    _moveHistory.add({
    ...
    if (hitBlot) {
      setState(() => _hitFlashBar = -1); // opp bar flashes
      Future.delayed(const Duration(milliseconds: 700), () {
        if (mounted) setState(() => _hitFlashBar = null);
      });
    }
  ...
  Future<void> _deductBetCoins() async {
    ...
    try {
      await http.post(
        Uri.parse(
            'https://1A15A2.playfabapi.com/Client/SubtractUserVirtualCurrency'),
        headers: {
  ...
  Future<void> _awardXPAndCoins({required bool iWon}) async {
    ...
    try {
      final res = await http.post(
        ...
        body: json.encode({
          "Keys": ["TotalXP", "DailyGamesPlayed", "LastGameDate", "Wins", "Losses"]
        }),
      );
      ...
      if (res.statusCode == 200) {
        ...
        dailyGames =
            int.tryParse(data?['DailyGamesPlayed']?['Value'] ?? '0') ?? 0;
      ...
      await http.post(
        Uri.parse(
            'https://1A15A2.playfabapi.com/Client/UpdatePlayerStatistics'),
        headers: {
          'Content-Type': 'application/json',
          'X-Authorization': widget.sessionTicket
        },
        body: json.encode({
          "Statistics": [
            {"StatisticName": "TotalXP", "Value": newTotalXP}
          ]
        }),
      );
      ...
      if (iWon) {
        await http.post(
          Uri.parse(
              'https://1A15A2.playfabapi.com/Client/AddUserVirtualCurrency'),
          headers: {
            'Content-Type': 'application/json',
            'X-Authorization': widget.sessionTicket
          },
          body: json.encode(
              {"VirtualCurrency": "CO", "Amount": widget.betAmount * 2}),
        );
      }
        ...
        child: LayoutBuilder(
          builder: (context, constraints) {
            ...
            return Stack(
              children: [
                Positioned.fill(
                    child: Image.asset('assets/background_dark.png',
                        fit: BoxFit.cover)),
                Row(
                  children: [
                    SizedBox(
                        width: width * 0.18, child: _buildMyPanel()),
                    Expanded(
                      child: Padding(
                    ...
                    SizedBox(
                        width: width * 0.18, child: _buildOpponentPanel()),
                  ],
                ),
                ...
                if (_gameState == "playing" &&
                    _currentTurnId == widget.myPlayFabId &&
                    _hasRolledThisTurn &&
                    (_availableMoves.isEmpty || !_hasAnyValidMove()) &&
                    !_isActionLocked)
                  Positioned(
                    bottom: height * 0.04,
                    right: width * 0.18 + 8,
  ...
  Widget _buildMyPanel() {
    bool isMyTurn =
        _currentTurnId == widget.myPlayFabId && _gameState == "playing";
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          // flip board button (replaces לפרוש)
          GestureDetector(
            onTap: _flipBoard,
            child: AnimatedContainer(
              ...
              decoration: BoxDecoration(
                gradient: LinearGradient(
                  colors: _isBoardFlipped
                      ? [const Color(0xFF4A148C), const Color(0xFF7B1FA2)]
                      : [const Color(0xFF1A237E), const Color(0xFF283593)],
                ),
                borderRadius: BorderRadius.circular(10),
                border: Border.all(
                    color: _isBoardFlipped
                        ? Colors.purpleAccent
              ...
              child: Column(
                ...
                children: [
                  Icon(
                    _isBoardFlipped ? Icons.flip_camera_android : Icons.flip,
                    color: Colors.white,
                    size: 18,
                  ),
  ...
  Widget _buildOpponentPanel() {
    bool isOpponentTurn =
        _currentTurnId == widget.opponentId && _gameState == "playing";
    return Container(
      color: Colors.black.withValues(alpha:0.5),
      child: Column(
        mainAxisAlignment: MainAxisAlignment.center,
        children: [
          AnimatedContainer(
            ...
            decoration: BoxDecoration(
              shape: BoxShape.circle,
              border: Border.all(
                  color: isOpponentTurn ? Colors.redAccent : Colors.white,
                  width: 3),
          ...
          Row(
            ...
            children: [
              Icon(Icons.close,
                  color: _opponentStrikes >= 1
                      ? Colors.red
                      : Colors.grey.withValues(alpha:0.5),
                  size: 30),
              const SizedBox(width: 5),
              Icon(Icons.close,
                  color: _opponentStrikes >= 2
                      ? Colors.red
                      : Colors.grey.withValues(alpha:0.5),
                  size: 30),
            ],
              ...
              Row(
                ...
                children: [
                  Column(children: [
                    ...
                    _buildDiceWidget(_myInitialRoll),
                  ]),
                  ...
                  Column(children: [
                    const Text("יריב",
                        style: TextStyle(color: Colors.white)),
                    const SizedBox(height: 8),
                    _buildDiceWidget(_opponentInitialRoll),
                  ]),
                ],
              ),
              const SizedBox(height: 20),
          ...
          child: AnimatedBuilder(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                ...
                border:
                    Border.all(color: const Color(0xFF2E1C11), width: 3),
              ...
              child: Container(
                ...
                child: LayoutBuilder(
                  builder: (ctx, innerC) {
                    ...
                    final double pointAreaW =
                        innerC.maxWidth - barW - bornOffW;
                    final double pointW = pointAreaW / 12;
                    final double colH = (innerC.maxHeight - 42) / 2;
                    final double cs =
                        min(pointW * 0.86, colH / 5.4).clamp(10.0, 36.0);

                    return _isBoardFlipped
                        ? _buildFlippedBoardRow(barW, bornOffW, cs, colH)
                        : _buildNormalBoardRow(barW, bornOffW, cs, colH);
                  },
                ),
              ),
            ),
  ...
  // ─── normal board layout ──────────────────────────────────────────────────
  Widget _buildNormalBoardRow(
      double barW, double bornOffW, double cs, double colH) {
    return Row(
      children: [
        // left half
        Expanded(
              ...
              Expanded(
                  child: Row(
                      children: List.generate(
                          6, (i) => _buildPoint(12 + i, isTop: true, cs: cs)))),
              SizedBox(
                  height: 42,
                  child: Center(child: _buildDiceArea(cs: cs))),
              Expanded(
                  child: Row(
                      children: List.generate(
                          6,
                          (i) =>
                              _buildPoint(11 - i, isTop: false, cs: cs)))),
            ],
          ),
        ),
        // bar
        _buildBar(barW, cs, oppOnTop: true),
        // right half (home side)
        Expanded(
          child: Column(
            children: [
              Expanded(
                  child: Row(
                      children: List.generate(
                          6,
                          (i) =>
                              _buildPoint(18 + i, isTop: true, cs: cs)))),
              ...
              Expanded(
                  child: Row(
                      children: List.generate(
                          6,
                          (i) =>
                              _buildPoint(5 - i, isTop: false, cs: cs)))),
            ],
          ),
        ),
        // borne-off area
        _buildBorneOffArea(bornOffW, cs, showMine: true),
      ],
    );
  }

  // ─── flipped board layout ─────────────────────────────────────────────────
  Widget _buildFlippedBoardRow(
      double barW, double bornOffW, double cs, double colH) {
    // Bot's home (18-23) appears at bottom right; human's home (0-5) appears at top right
    return Row(
      children: [
        // left half (flipped)
        Expanded(
          child: Column(
            children: [
              Expanded(
                  child: Row(
                      children: List.generate(
                          6, (i) => _buildPoint(5 - i, isTop: true, cs: cs)))),
              SizedBox(
                  height: 42,
                  child: Center(child: _buildDiceArea(cs: cs))),
              Expanded(
                  child: Row(
                      children: List.generate(
                          6,
                          (i) =>
                              _buildPoint(12 + i, isTop: false, cs: cs)))),
            ],
          ),
        ),
        // bar (swapped)
        _buildBar(barW, cs, oppOnTop: false),
        // right half (flipped home side)
        Expanded(
          child: Column(
            children: [
              Expanded(
                  child: Row(
                      children: List.generate(
                          6,
                          (i) =>
                              _buildPoint(11 - i, isTop: true, cs: cs)))),
              const SizedBox(height: 42),
              Expanded(
                  child: Row(
                      children: List.generate(
                          6,
                          (i) =>
                              _buildPoint(18 + i, isTop: false, cs: cs)))),
            ],
          ),
        ),
        // borne-off area (shows opponent in flipped view)
        _buildBorneOffArea(bornOffW, cs, showMine: false),
      ],
    );
  }

  // ─── bar widget ───────────────────────────────────────────────────────────
  Widget _buildBar(double barW, double cs, {required bool oppOnTop}) {
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        child: Column(
          ...
          children: [
            // top = opp bar (in normal) or my bar (flipped)
            if (oppOnTop && _oppBar > 0)
              ...List.generate(
                  _oppBar, (i) => _buildChecker(false, size: cs)),
            if (!oppOnTop && _myBar > 0)
              ...List.generate(
                  _myBar, (i) => _buildChecker(true, size: cs)),
            // selection indicator
            ...
            // bottom = my bar (normal) or opp bar (flipped)
            if (oppOnTop && _myBar > 0)
              ...List.generate(
                  _myBar, (i) => _buildChecker(true, size: cs)),
            if (!oppOnTop && _oppBar > 0)
  ...
  Widget _buildBorneOffArea(double bornOffW, double cs,
    ...
    final int count = showMine ? _myBorneOff : _oppBorneOff;
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: const Border(
              left: BorderSide(color: Color(0xFF2E1C11), width: 4)),
          boxShadow: [
            BoxShadow(
        ...
        child: Column(
          ...
          children: [
            if (_validDestinations.contains(-1) && showMine)
              AnimatedContainer(
                duration: const Duration(milliseconds: 200),
                height: 44,
            ...
            Expanded(
              child: Center(
                child: Column(
                  ...
                  children: [
                    Icon(
                      showMine ? Icons.person : Icons.person_outline,
  ...
  Widget _buildPoint(int index, {required bool isTop, required double cs}) {
    ...
    bool isHighlighted =
        (_highlightFrom == index || _highlightTo == index);
          ...
          child: CustomPaint(
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: List.generate(
                    checkersCount > 5 ? 5 : checkersCount, (i) {
                  if (i == 4 && checkersCount > 5) {
                    return _buildChecker(isMine,
                        isSelected: isSelected && i == 4,
                        extraCount: checkersCount - 4,
                        size: cs);
                  }
                  return _buildChecker(isMine,
                      isSelected: isSelected && i == checkersCount - 1,
                      size: cs);
                }),
              ),
            ),
  ...
  // ─── checker widget ───────────────────────────────────────────────────────
  Widget _buildChecker(bool isMine,
      {bool isSelected = false, int extraCount = 0, double size = 30}) {
    final checkerWidget = Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
          color: isSelected
              ? Colors.yellow
              : (isMine ? Colors.grey.shade400 : Colors.black87),
      ...
      child: Center(
        child: Container(
          ...
          decoration: BoxDecoration(
            shape: BoxShape.circle,
            border: Border.all(
                color: isMine ? Colors.grey.shade300 : Colors.black38,
                width: 1.2),
          ),
          child: extraCount > 0
              ? Center(
  ...
  Widget _buildDiceArea({double cs = 30}) {
    ...
    if (_currentTurnId == widget.myPlayFabId) {
        ...
        return Row(mainAxisSize: MainAxisSize.min, children: [
          ...
          TweenAnimationBuilder<double>(
            ...
            child: ElevatedButton(
              ...
              onPressed: (_isRolling || _isActionLocked)
                  ? null
                  : _rollPlayingDice,
              child: _isRolling
                  ? const SizedBox(
      ...
      // after roll – show dice with consumed state
      return _buildDiceRow(diceSize, forBot: false);
    }
    ...
    return _buildDiceRow(diceSize, forBot: true);
  }

  Widget _buildDiceRow(double diceSize, {required bool forBot}) {
    if (_die1 == _die2) {
      // Doubles: show 4 dice
      final int remaining =
          _availableMoves.where((m) => m == _die1).length;
      final double smallDice = (diceSize * 0.82).clamp(18.0, 34.0);
      return Row(mainAxisSize: MainAxisSize.min, children: [
        _buildDiceWidget(_die1, size: smallDice, consumed: remaining < 4),
        const SizedBox(width: 3),
        _buildDiceWidget(_die1, size: smallDice, consumed: remaining < 3),
        const SizedBox(width: 3),
        _buildDiceWidget(_die1, size: smallDice, consumed: remaining < 2),
        const SizedBox(width: 3),
        _buildDiceWidget(_die1, size: smallDice, consumed: remaining < 1),
      ]);
    }
    return Row(mainAxisSize: MainAxisSize.min, children: [
      _buildDiceWidget(_die1,
          size: diceSize,
          consumed: _hasRolledThisTurn && !_availableMoves.contains(_die1)),
      const SizedBox(width: 8),
      _buildDiceWidget(_die2,
          size: diceSize,
  ...
  // ─── dice visual widget (with shake during roll) ──────────────────────────
  Widget _buildDiceWidget(int value, {double size = 50, bool consumed = false}) {
    final dice = Opacity(
      opacity: consumed ? 0.28 : 1.0,
      child: Container(
        width: size,
        height: size,
        decoration: BoxDecoration(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: Column(
                        children: [
                          ...
                          const SizedBox(height: 4),
                          Text(
                              "רמה $_levelAfter  •  $xpInLevel / $xpNeeded XP",
                              style: const TextStyle(
                    ...
                    ElevatedButton(
                      style: ElevatedButton.styleFrom(
                        backgroundColor:
                            _winnerIsMe ? Colors.amber : Colors.blueGrey,
                        padding: const EdgeInsets.symmetric(
                      ...
                      child: Text(
                        ...
                        style: TextStyle(
                            ...
                            fontWeight: FontWeight.bold,
                            color:
                                _winnerIsMe ? Colors.black : Colors.white),
                      ),
                    ),
                  ],
                ),
